...
// Define the type for the App component as a Functional Component
const App: FC = () => {
  ...
  // Function to fetch recommended videos using AI and YouTube API
  const fetchRecommendedVideo = async () => {
    try {
      ...
      const searchQuery: string = geminiResult.candidates?.[0]?.content?.parts?.[0]?.text || '';
      ...
      const videos: Video[] = youtubeResult.items.map((item: any) => ({
        id: item.id.videoId,
  ...
  // New function to generate a personalized health report
  ...
  // New function to fetch detailed symptom information
  ...
  // Event handler for radio buttons and input fields
    ...
    <div className="min-h-screen bg-gray-100 p-4 flex items-center justify-center font-sans">
      <div className="max-w-4xl w-full bg-white rounded-xl shadow-lg p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {/* 2. 통증 좌우 선택 (손목에 한정) - Conditional Rendering */}
          {symptom === '손목터널 증후군' && (
            <div className="bg-gray-50 p-6 rounded-lg shadow-inner">
          ...
          <div className="bg-gray-50 p-6 rounded-lg shadow-inner">
            ...
            <div className="space-y-2">
              {['약간 아픈 정도', '일상생활에 지장이 있는 정도', '참기 힘든 정도'].map((level, index) => (
                <label key={index} className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="radio"
                    name="painLevel"
        ...
        {/* Recommendation Section (Conditionally rendered) */}
        {error && (
          <div className="mt-8 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-xl relative" role="alert">
            <span className="block sm:inline">{error}</span>
          </div>
        )}

        {/* New sections for Health Report and Symptom Info */}
        {(healthReport || symptomInfo) && (
