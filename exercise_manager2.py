...
def load_user_data_from_supabase(user_id):
    """Supabase에서 사용자의 모든 기록 데이터를 불러옵니다."""
    try:
        # 'user_data' 테이블에서 user_id에 해당하는 모든 데이터를 가져옵니다.
        # 이 테이블은 운동 기록과 통증 데이터를 모두 포함한다고 가정합니다.
        response = supabase.table('user_data').select('*').eq('user_id', user_id).order('date', desc=False).execute()
