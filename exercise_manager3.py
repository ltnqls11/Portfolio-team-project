...
def load_user_data_from_supabase(user_id, table_name):
    ...
    if not SUPABASE_ENABLED:
        # Supabase가 비활성화된 경우, 임시 데이터를 반환하여 오류를 방지합니다.
        # Streamlit 세션 상태를 사용해 임시로 데이터를 저장합니다.
    ...
    try:
        # 'user_data' 테이블에서 user_id에 해당하는 모든 데이터를 가져옵니다.
        # 이 테이블은 운동 기록과 통증 데이터를 모두 포함한다고 가정합니다.
        response = supabase.table(table_name).select('*').eq('user_id', user_id).order('date', desc=False).execute()
...
def save_exercise_log_to_supabase(user_id, log_data):
    ...
    if not SUPABASE_ENABLED:
        # Supabase가 비활성화된 경우, 로컬 임시 데이터에 저장
    ...
    try:
        ...
        # Upsert: 이미 존재하는 데이터는 업데이트하고, 없으면 새로 추가합니다.
...
def show_integrated_dashboard(user_id):
    ...
    if all_exercise_data or all_pain_data:
        ...
        if not pain_df.empty:
            ...
            pain_df = pain_df.set_index('date').sort_index()
        ...
        if not exercise_df.empty or not pain_df.empty:
            ...
            if not pain_df.empty and 'user_id' in pain_df.columns:
                pain_df = pain_df.drop(columns=['user_id'])
            
            combined_df = pd.concat([pain_df, exercise_df], axis=1).fillna(0)
