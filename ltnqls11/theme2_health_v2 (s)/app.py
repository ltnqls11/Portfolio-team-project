...
def show_notification_setup():
    ...
    **💡 알림 내용:** 개인 맞춤 운동 루틴 및 스트레칭 가이드
    ...
    with col1:
        if st.button("🔍 n8n 워크플로우 상태 확인"):
            n8n_webhook_url = "https://n8n-server-u54574.vm.elestio.app/webhook/5a133ebe-efdf-4797-8852-0faca4cc939c/chat"
            ...
            try:
                ...
                st.info("🔄 테스트 요청을 보내는 중...")
                response = requests.post(n8n_webhook_url, json=test_data, timeout=10)
                
                if response.status_code == 200:
                    st.success("✅ n8n 워크플로우가 활성화되어 있습니다!")
                    try:
                        response_data = response.json()
                        st.info(f"📋 응답: {response_data}")
                    except:
                        st.info(f"📋 응답 텍스트: {response.text}")
                        
                elif response.status_code == 404:
                    st.error("❌ 워크플로우를 찾을 수 없습니다 (404)")
                    st.error("n8n에서 워크플로우를 활성화해주세요.")
                    
                elif response.status_code == 500:
                    st.error("❌ n8n 서버 내부 오류 (500)")
                    st.error("워크플로우 설정을 확인해주세요.")
                    try:
                        error_data = response.json()
                        st.error(f"📄 오류 내용: {error_data}")
                    except:
                        st.error(f"📄 오류 텍스트: {response.text}")
                        
                else:
                    st.warning(f"⚠️ 예상치 못한 응답: HTTP {response.status_code}")
                    st.warning(f"📄 응답: {response.text}")
                    
            except requests.exceptions.ConnectionError:
                st.error("❌ n8n 서버에 연결할 수 없습니다")
    ...
    with col2:
        if st.button("🚀 알리미 활성화", type="primary"):
            ...
            webhook_data = {
                ...
                "conditions": list(st.session_state.selected_conditions) if st.session_state.selected_conditions else [],
                "pain_scores": dict(st.session_state.user_data.get('pain_scores', {})),
                "slack_webhook": slack_webhook if slack_webhook else None,
                "created_at": datetime.now().isoformat(),
                "user_name": st.session_state.user_data.get('name', ''),
                "user_age": st.session_state.user_data.get('age', 0),
                "work_intensity": st.session_state.user_data.get('work_intensity', '보통')
            }
            
            try:
                # n8n webhook 호출
                st.info("🔄 n8n 워크플로우를 호출하고 있습니다...")
                ...
                headers = {
                    ...
                    'User-Agent': 'VDT-Management-System/1.0'
                }
                
                response = requests.post(n8n_webhook_url, json=webhook_data, headers=headers, timeout=15)
                
                # 응답 상태 코드별 처리
                if response.status_code == 200:
                    st.success("✅ n8n 워크플로우가 성공적으로 실행되었습니다!")
                    st.success("📧 Gmail과 Slack으로 자동 알림이 설정되었습니다!")
                    
                    # 응답 내용 표시 (있는 경우)
                    try:
                        response_data = response.json()
                        if response_data:
                            st.info(f"📋 워크플로우 응답: {response_data}")
                    except:
                        st.info(f"📋 응답 텍스트: {response.text}")
                    
                elif response.status_code == 404:
                    st.error("❌ n8n 워크플로우를 찾을 수 없습니다 (HTTP 404)")
                    st.error("🔧 해결 방법:")
                    st.error("1. n8n에서 워크플로우가 활성화되어 있는지 확인")
                    st.error("2. webhook URL이 올바른지 확인")
                    st.error("3. n8n 서버가 실행 중인지 확인")
                    st.code(f"URL: {n8n_webhook_url}")
                    
                elif response.status_code == 500:
                    st.error("❌ n8n 서버 내부 오류 (HTTP 500)")
                    st.error("🔧 해결 방법:")
                    st.error("1. n8n 워크플로우 내부 로직을 확인해주세요")
                    st.error("2. 워크플로우에서 처리할 수 없는 데이터가 있을 수 있습니다")
                    st.error("3. n8n 로그를 확인해보세요")
                    ...
                    # 로컬 설정은 계속 저장
                    st.warning("⚠️ n8n 연결 실패했지만 로컬 설정은 저장됩니다.")
                    
                else:
                    st.error(f"❌ n8n 워크플로우 호출 실패: HTTP {response.status_code}")
                ...
                notification_config = {
                    ...
                    "pain_scores": st.session_state.user_data.get('pain_scores', {}),
                    "created_at": datetime.now().isoformat(),
                    "n8n_webhook_called": response.status_code == 200,
                    "n8n_response_code": response.status_code
                }
                ...
                st.success("💾 로컬 설정이 저장되었습니다!")
                        
            except requests.exceptions.Timeout:
                st.error("❌ n8n 서버 응답 시간 초과 (15초)")
                st.error("🔧 n8n 서버가 응답하지 않습니다. 서버 상태를 확인해주세요.")
            except requests.exceptions.ConnectionError:
                st.error("❌ n8n 서버에 연결할 수 없습니다")
                st.error("🔧 네트워크 연결 또는 서버 상태를 확인해주세요.")
            except Exception as e:
                st.error(f"❌ n8n 워크플로우 호출 중 오류 발생: {str(e)}")
                
            # 오류가 발생해도 로컬 설정은 저장
            if 'notification_config' not in locals():
                try:
                    notification_config = {
                        "type": notification_type, 
                        "email": user_email,
                    ...
                    st.info("💾 로컬 알림 설정이 저장되었습니다.")
                except Exception as save_error:
                    st.error(f"❌ 설정 저장 실패: {str(save_error)}")
